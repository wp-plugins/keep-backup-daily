...
<p><strong>'.$settings['notification'].'</strong></p></div>':'';
		...
		$expected_backup = $_SESSION['expected_backup']!=''?$_SESSION['expected_backup']:($_SESSION['expected_backup']=@file_get_contents('http://www.androidbubbles.com/api/kbd.php?next_backup='.time().'&backup_time='.$settings['backup_required'].'&domain_url='.base64_encode($submitted_url)));
